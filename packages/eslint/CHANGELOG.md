...
- e730cb8: Replace `eslint-plugin-react` with `eslint-plugin-react-x`, unblocking ESLint 10

    `eslint-plugin-react` has no release that supports ESLint 10 — its peer range
    stops at `^9.7`, and the `next` dist-tag (`7.8.0-rc.0`) is semver _older_ than
    ...
    **Overlap with `eslint-plugin-react-hooks`**
    ...
    resolves this the _other_ way round — it disables the `react-hooks` side. We
